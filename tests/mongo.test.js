...
describe('# MongoDB', () => {
  ...
  describe('# 修改记录', () => {
    ...
    beforeAll(async () => {
      ...
      const user = await db.save(User, record)
      userID = user.id
    })

    test('# 改db.save()，修改基本类型字段（字符串）', async () => {
      await db.saveOne(User, userID, { password: 'iiii' })
      expect(await db.select(User, { _index: userID })).toHaveProperty(
        'password',
        'iiii'
      )
    })
    ...
    test('# 改db.save()，修改基本类型字段，用save函数', async () => {
      await db.save(User, { password: 'yyyy' }, { _index: userID })
      expect(await db.select(User, { _index: userID })).toHaveProperty(
        'password',
        'yyyy'
      )
    ...
    test('# 改db.save()，修改基本类型字段（数字）', async () => {
      await db.saveOne(User, userID, { age: 23 })
      expect(await db.select(User, { _index: userID })).toHaveProperty(
        'age',
        23
      )
    ...
    test('# 改db.save()，修改数组类型字段（元素）', async () => {
      await db.saveOne(User, userID, { tags: 12 })
      const user = await db.select(User, { _index: userID })
      expect(user.tags).toContain(12)
    })

    test('# 改db.save()，修改数组类型字段（数组）', async () => {
      await db.saveOne(User, userID, { tags: ['hhhh', '7777'] })
      const user = await db.select(User, { _index: userID })
      // 修改之后记录数据刷新
    ...
    test('# 改db.save()，追加数组类型字段（元素）', async () => {
      await db.saveOne(User, userID, { tags: 100 }, { updMode: 'append' })
      const user = await db.select(User, { _index: userID })
      // 追加之后记录数据刷新
      expect(user.tags).toContain('hhhh')
      expect(user.tags).toContain('7777')
    ...
    test('# 改db.save()，删除数组类型字段的元素', async () => {
      await db.saveOne(User, userID, { tags: 'hhhh' }, { updMode: 'delete' })
      const user = await db.select(User, { _index: userID })
      // 删除之后记录数据刷新
    ...
    test('# 改db.save()，修改子字段（字符串）', async () => {
      await db.saveOne(User, userID, { 'subItem.name': 'aaaa' })
      const user = await db.select(User, { _index: userID })
      // 修改之后记录数据刷新
    ...
    test('# 改db.save()，修改子字段（数字）', async () => {
      await db.saveOne(User, userID, { 'subItem.num': 12 })
      const user = await db.select(User, { _index: userID })
      // 修改之后记录数据刷新
    ...
    describe('# 改db.save()，修改多记录', () => {
      ...
      test('# 数据清除后再修改', async () => {
        await db.save(User, { age: 50 }, { password: 'frfrfr' })

        const users = await db.select(User, { password: 'frfrfr' })
  ...
  describe('# 查询记录', () => {
    ...
    test('# 查db.select()，指定id', async () => {
      expect(await db.select(User, { _index: user.id })).toHaveProperty(
        'username',
        user.username
      )
    })
    ...
    test('# 查db.select()，排序', async () => {
      const users = await db.select(User, { order_by: { age: -1 } })
