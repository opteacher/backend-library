...
describe('# 生成路由', () => {
  ...
  describe('# 接口路由', () => {
    test('# 接口测试', async () => {
      let resp = await request.get('/backend-library/api/v1/test').expect(200)
      let result = resp.body.result
      expect(result.message).toEqual('Hello World')
      resp = await request.get('/backend-library/api/v1/test/active').expect(200)
      result = resp.body.result
      expect(result.message).toEqual('Hello Active')
    })
  })
})
